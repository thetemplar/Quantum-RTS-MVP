  ...
  private void OnDrawGizmos() {
    ...
    if (config2D != null) {
      ...
      if (config2D.ShapeType == Shape2DType.Polygon) {
        ...
        if (collider) {
          QuantumGameGizmos.DrawShape2DGizmo(Shape2D.CreatePolygon((PolygonCollider)collider.AssetObject, config2D.PositionOffset, FP.FromRaw((config2D.RotationOffset.RawValue * FP.Raw.Deg2Rad) >> FPLut.PRECISION)), position2D, rotation2DDeg.ToUnityQuaternionDegrees(), color, height, null);
        }
